...
async function loadProducts() {
    try {
        ...
        const productPromises = skus.map(sku => fetch(`${API_BASE}/inventory/${sku}`).then(r => r.json()));
...
function renderProducts() {
    ...
    container.innerHTML = products.map(product => `
        <div class="product-card">
            ...
            <div class="quantity ${getQuantityClass(product.availableQuantity)}">
                Available: ${product.availableQuantity}
            </div>
            <div class="stock-indicator ${getStockClass(product.availableQuantity)}">
            ...
            >
                ${product.availableQuantity === 0 ? 'Out of Stock' : 'Reserve Now'}
            </button>
        </div>
    `).join('');
}
...
async function reserveProduct(sku, quantity) {
    try {
        ...
        if (result.success) {
            ...
            showMessage(`Reservation successful! You have 5 minutes to confirm.`, 'success');
            showReservationInfo();
            startCountdown();
...
function showReservationInfo() {
    ...
    details.innerHTML = `
        <p><strong>Product:</strong> ${formatProductName(currentReservation.sku)}</p>
...
function startCountdown() {
    ...
    function updateCountdown() {
        ...
        if (diff <= 0) {
            ...
            clearInterval(countdownInterval);
            showMessage('Your reservation has expired. Please reserve again.', 'error');
